...
uniquely identifying the origin of the tuple.
...
When a bolt is eligible to snapshot its state, it forks a thread to perform the
save. On boot-up, a bolt tries to restore its state from the provided
...
processors in the topology that are allowed to execute impure code).  There is
no restriction to the type of computation performed on incoming tuples;
however, since no snapshots are performed on sinks, non-idempotent operations
