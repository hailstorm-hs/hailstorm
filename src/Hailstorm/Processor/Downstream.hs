...
runDownstream :: (Topology t, SnapshotStore s)
              => ZKOptions
              -> ProcessorId
              -> t
              -> s
              -> IO ()
runDownstream opts dId@(dName, dInst) topology snapshotStore = do
    ...
    let (_, port) = addressFor topology dId
        ...
        startState = case pr of
                         SinkNode _ -> SinkRunning
                         BoltNode _ -> BoltLoaded savedClk
                         _ -> throwNoDownstreamError
        processSocket s pcOutput = runEffect $
            producer s >-> PC.toOutput pcOutput

    groundhogDay (runDownstream opts dId topology snapshotStore) $ do
